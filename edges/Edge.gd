...
var orientation
signal edge_entered(orientation, body)
...
func _on_collision_area_body_entered(body):
    #print_debug(body.name + " entered: " + self.name )
    call_deferred("emit_signal","edge_entered",orientation, body)
...
func _determine_side_exited(body_position : Vector2):
    var distance_to_inner_side : int = body_position.distance_to(inner_side)
    var distance_to_outer_side : int = body_position.distance_to(outer_side)
